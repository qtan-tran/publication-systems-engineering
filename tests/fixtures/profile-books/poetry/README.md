# Synthetic Poetry Fixture

This is a Publication Systems Engineering project generated with the `poetry` profile.
...
`poetry` is a generator-supported profile. Use `config/pse-local.tex` for restrained title-local exceptions rather than editing the shared core or profile package.

## Poetry semantics

This project uses stable stanza and verse-line identifiers from the `verse-structure` semantic module. Indentation, hanging continuation, stanza spacing, and display numbering belong to the `poetry` profile; semantic line identity belongs in title-owned source.
